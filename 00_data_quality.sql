...
--           Real credit risk work starts here — garbage in,
--           garbage out applies hard in regulated environments.
...
-- Finding: identify which features will need imputation or exclusion
SELECT
...
-- Finding: flag for exclusion in the feature engineering view
SELECT
...
-- Finding: borrowers where emp_length > age - 16 suggest data entry errors
...
-- A large discrepancy suggests the column is stored as a ratio (e.g. 0.15)
...
FROM credit_risk_dataset_2;
-- Finding: if avg_stored_value ≈ avg_derived_ratio the column is already a ratio (0–1).
-- Cast to DECIMAL(5,2) is appropriate. If it's ~100x larger, values are percentages.
...
-- risk proxy and all downstream segment analysis is compromised.
-- Finding: confirms whether loan_grade is usable as a risk signal.
SELECT
    loan_grade,
...
-- is corrupted and the entire analysis must be re-examined.
SELECT
    loan_grade,
