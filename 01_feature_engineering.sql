...
-- Purpose : Build the analytical base view used by all
--           downstream queries. Applies validated exclusions
--           from 00_data_quality.sql and derives features
--           with explicit business rationale for each.
-- ============================================================
...
SELECT
    ...
    -- LGD = assumed 45% (Basel II unsecured consumer default)
    -- EAD = loan_amnt
    -- For portfolio-level analysis, substitute AVG(loan_status)
    -- as the segment PD — see 04_advanced_analysis.sql.
    -- --------------------------------------------------------
    ...
    -- adjust for US dataset if required.
    -- --------------------------------------------------------
    CASE
    ...
    -- Validated in 03_segment_analysis.sql — each tier should
